...
export default function TeamDetail() {
  ...
  // const canManageMembers = isOwner || permissions.members?.invite; // Replaced by hasPermission
  // const canEditMembers = isOwner || permissions.members?.edit; // Replaced by hasPermission
  // const canManagePlugins = isOwner || permissions.plugins?.view; // Replaced by hasPermission
  // const canManageSites = isOwner || permissions.sites?.view; // Replaced by hasPermission
  // const canCreateNotifications = isOwner || permissions.notifications?.create; // Replaced by hasPermission
  // const canDeleteNotifications = isOwner || permissions.notifications?.delete; // Replaced by hasPermission
  // const canViewNotifications = isOwner || (permissions.notifications?.view ?? true); // Replaced by hasPermission
  // const canViewInbox = isOwner || (permissions.inbox?.view ?? true); // Replaced by hasPermission
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          {(isOwner || hasPermission('inbox', 'view')) && (
            <Popover open={teamInboxOpen} onOpenChange={setTeamInboxOpen}>
              ...
              <PopoverContent className="w-96 p-0" align="end">
                <div className="p-4 border-b border-gray-100">
                  <h3 className="font-semibold text-gray-900">Team Inbox</h3>
                  <p className="text-xs text-gray-500 mt-1">{unreadTeamMessages} ongelezen</p>
                </div>
                        ...
                        {teamInboxMessages.map((message) => (
                          <div
                            ...
                            className={`p-3 rounded-lg transition-colors ${
                              message.is_read ? 'bg-white hover:bg-gray-50' : 'bg-blue-50 hover:bg-blue-100'
                            }`}
                          >
